...
> **In simple words — what this page teaches:** Turn the full design into a clear two-minute story, then expand it into a deeper interview explanation without listing tools blindly.
...
>
> On the offline path, S3 events feed a durable metadata queue, while the heavy ingestion path separately accepts a request, creates one task per file, throttles work, and launches idempotent Airflow workers. A file-arrival event updates catalog state; it does not claim that processing is complete. Using range reads through fsspec, s3fs, or gcsfs, we extract EBCDIC textual headers, binary and selected trace metadata, then combine that with ingestion, QC, lineage, and delivery information. We normalize vendor-specific fields into a versioned schema, quarantine invalid records, create source-aware chunks, generate embeddings, and publish versioned OpenSearch indexes.
>
...
>
> We evaluate extraction, retrieval, and generation separately using a seismic-engineer golden set, with Recall@K, MRR/NDCG, faithfulness, citation accuracy, latency, and cost. We propagate OpenTelemetry context through HTTP, queues, task records, Airflow, and workers; join traces, logs, and metrics in Grafana by request and task ID; and use versioned indexes, circuit breakers, backoff, and canary rollback for reliability.
...
Event notification for metadata freshness plus scheduled reconciliation for correctness. The event consumer is idempotent and short-running. Heavy ingestion is separately requested, persisted, throttled, and tracked. Identity includes immutable object version; key alone is insufficient.
...
Stateless FastAPI, durable request/task state, global and per-product throttling, autoscaling, probes, gradual rollout, least-privilege IAM, and project filters before retrieval. Propagate trace context through HTTP, queue, database, Airflow, and worker boundaries; join OTLP traces/logs/metrics in Grafana by request and task ID.
